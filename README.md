...
48,900+ 行 Python · 654 个测试用例 · 24 个模块 · 5 层路由管道 · 28 个量化因子 · 31 条军规
...
```
CLI → 军规(31条) → 准入检查 → 多维诊断 → 综合裁决 → 仓位调度 → 风控执行
...
| 模块 | 路径 | 核心能力 |
|------|------|---------|
| 🛤️ 路由 | `src/routing/` | 5 层管道编排 + Agent-Worker 模式 + 护栏执行器 + 技术分析 + 入场出场引擎 |
| 🔍 Alpha Lens | `src/alpha/` | 信息层级判定、共识缺口检测、叙事生命周期定位、Alpha 衰减追踪 |
| 🗄️ 数据层 | `src/data/` | 5 源聚合 + 交叉验证 + TTL 缓存 + 分钟/tick 级行情 |
...
| 📈 宏观 | `src/macro/` | 货币-信用双象限框架（社融/M1-M2/DR007/LPR）+ 财政政策视角 |
