...
A 股最致命的不是买贵了，是**追了已经被充分定价的"好故事"**。白泽独创四维 Alpha 评估引擎：
...
| **🍃 紫苏叶理论** | 标的是金枪鱼还是紫苏叶？ | 供应链深度 Alpha — 离终端 ≥ 2 层的上游隐蔽关键环节，被忽视但不可替代 |

> 顶级寿司店里食客都在抢金枪鱼大腹（GPU/云计算/终端龙头），但真正让整家店停摆的不是最贵的鱼肉，而是紫苏叶——离终端 2-3 层、被忽视但不可替代的上游环节。紫苏叶标的在市场尚未充分发现时额外加分。
...
| **四视角辩论** | AI Berkshire 四大师（巴菲特/芒格/索罗斯/林奇）独立评分 + 分歧检测 |
| **Munger 232 思维模型** | 从 232 个芒格思维模型中动态匹配最相关的 3-5 个辅助判断 |
...
系统内置四类完整分析场景，禁止跳过管道阶段：

| 场景 | 触发词 | 核心流程 |
|------|--------|---------|
| **荐股**（有主题/无主题） | 推荐股票、选股、有什么票可以买 | 信息搜集 → 数据初筛 → 管道并行验证 → 横向排名 |
| **个股涨跌归因** | XX为什么涨停/跌停/大涨/大跌 | 信息搜集 → 多维归因 → 因果推断（主因→次因→噪音） |
| **大盘涨跌归因** | 今天A股为什么大跌/大涨 | 信息搜集 → 结构分析 → 8通道因果推断 → 应对建议 |
| **主题/单票深度分析** | XX主题分析、XX能买吗 | 信息搜集 → 量化分析 → 全链路管道输出 |

详见 [`.claude/CLAUDE.md`](.claude/CLAUDE.md) — 用户交互 Workflow 调度章节。
...
| 🛡️ 持仓哨兵 | `src/sentinel/` | 轻量盘中预警 + 多频道微信推送（cron 友好） |
...
# 🔎 高 Alpha 股票扫描
python -m src alpha-scan --min-alpha 60

# 📉 Alpha 衰减追踪
python -m src alpha-decay 600519
...
# 🎲 博弈论全景
python -m src game-theory

# 🔍 个股涨跌归因（Phase 1 自动搜集 + 质量预检）
python -m src attribute 600089
```
...
```bash
# 📊 持仓监控
python -m src position-monitor status  # 所有持仓摘要
python -m src pm check                 # 实时检查 + 止损更新

# 🏭 深度研究
python -m src sector-research 半导体    # 行业全景研究
...
```bash
# 🏟️ 策略竞技场
python -m src arena benchmark          # 快速基准（全部预置策略）
python -m src arena run                # 运行策略竞技场对比

# 📝 投研笔记
python -m src note add --topic "AI算力" --summary "..."
python -m src note search 算力          # 全文搜索

# 📊 交互日志
python -m src interaction-log stats    # 使用统计
```
...
投资者画像支持配置 `trading_style`（长线/波段/短线）、`holding_period`、`accessible_boards`（可交易板块），系统自动切换分析管道参数。
...
| Phase 4 | Alpha Lens 四维评估 + 策略进化 + 用户反馈闭环 + 政策 NLP + 信息速度监控 | ✅ |
...
| 文档 | 内容 |
|------|------|
| [`.claude/CLAUDE.md`](.claude/CLAUDE.md) | 项目架构 & 用户交互 Workflow 调度 & 开发工作流 |
