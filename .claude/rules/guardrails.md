...
- 连续跌停股票排除
- 有退市风险的股票提示
- **价格跌破 200 周均线的股票一律排除**（对应军规 r035，不等待基本面分析）

  > ⚠️ 数据不足 200 周时自动降级：≥100 周使用 N 周 MA 替代；≥50 周使用 N 周 MA 替代并降置信度；<50 周标记 [DATA_GAP] 不触发。替代 MA 的 r036 确认阈值提高至 8 周。
...
- **价格站回 200 周均线不足 4 周** — 趋势反转未确认，降低置信度（对应军规 r036）
