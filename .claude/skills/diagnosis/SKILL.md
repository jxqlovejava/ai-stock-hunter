...
- 瓶颈类型 (产能/技术/资源/牌照/无)
- 瓶颈**身份分** (0-100, cyberagent)
- **Serenity 研究轴**（自动 `apply_serenity_defaults`）:
  - `what_constrains` / `serenity_role`
  - `research_priority_score` 0–100（非买卖）
  - `failure_conditions` / `next_checks`
...
DiagnosisReport(
    ...
    bottleneck_analysis=BottleneckAnalysis(...),  # 含 research_priority_score
...
输出展示必须含 **卡点挑战** 行：卡住的环节、研究优先级、失败条件、下一步核验。
...
- 瓶颈分析: `src/industry/bottleneck.py`, `src/industry/serenity_scorecard.py`, `src/industry/serenity_workflow.py`
- 供应链: `src/industry/supply_chain.py`
- 因子数据: `src/data/factor_pipeline.py`
- 盈利修正: `src/data/earnings_revision.py`
- 依赖 Skill: `admission`, `verdict`, `macro-monitor`, `game-theory`, `serenity-bottleneck`
