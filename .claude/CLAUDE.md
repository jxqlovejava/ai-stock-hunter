...
| 3. 管道验证 | 对每支候选**并行**跑 `stock-hunter` 全链路（军规→准入→诊断→Alpha→质量→辩论→博弈→裁决→T+0→风控） | 并行 |
...
| 3. 因果推断 | **信息源质量检查**（T0-T3分级/时效性校验/STALE排除/交叉验证）→ macro event causality chain（8条传导通道）→ 主因→次因→噪音 排序 → 区分事实/解读/推测 | 串行 |
