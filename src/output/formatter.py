...
def format_analysis_result(result: OrchestratorResult) -> str:
    ...
    流程 (7 步):
      Step 1 🏥 军规审查        — 30条硬规则逐条核查
      Step 2 🚪 准入检查        — ST/次新/流动性/涨跌停
      Step 3 📊 多维诊断        — 宏观/价值/质量/动量/估值/周期/情绪/高管 8维度
      Step 4 🎭 四大师辩论      — 巴菲特/李录/芒格/林奇 多角度审视
      Step 5 ⚖️  综合裁决        — 加权评分+置信度+可证伪条件
      Step 6 💰 仓位调度        — 凯利公式/线性回退→目标仓位
      Step 7 🛡️ 风控执行        — 单票上限/止损/回撤/流动性 硬约束
    """
    lines: list[str] = []
    ...
    # ═══════════════════════════════════════════════════════════════
    # 📈 Alpha Lens + 博弈论 + 思维模型 (辅助分析)
    # ═══════════════════════════════════════════════════════════════
    alpha = result.alpha_profile
    gt = result.game_theory_info
    mm_models = result.mental_models

    if alpha or gt or mm_models:
        lines.append(f"\n{SEP_EQ}")
        lines.append("  🔬 辅助分析 — Alpha信息优势 + 博弈论 + 思维模型")
        lines.append(SEP_EQ)

    if alpha is not None:
        lines.append(f"  📈 Alpha Lens: {alpha.alpha_score:.0f}/100  "
    ...
    if mm_models:
        names = [f"{m.get('name_cn','')}[{m.get('discipline','')}]" for m in mm_models[:3]]
        lines.append(f"  🧩 思维模型: {', '.join(names)}")

    # ═══════════════════════════════════════════════════════════════
    # Step 5 ⚖️ 综合裁决 (原 L2 Judge)
    # ═══════════════════════════════════════════════════════════════
    lines.append(f"\n{SEP_EQ}")
    lines.append("  Step 5 ⚖️  综合裁决 — 加权评分+风险+可证伪条件")
    lines.append(SEP_EQ)
    ...
    # ═══════════════════════════════════════════════════════════════
    lines.append(f"\n{SEP_EQ}")
    lines.append("  Step 6 💰 仓位调度 — 信号→目标仓位")
    lines.append(SEP_EQ)
    ...
    # ═══════════════════════════════════════════════════════════════
    lines.append(f"\n{SEP_EQ}")
    lines.append("  Step 7 🛡️ 风控执行 — 硬约束裁剪")
    lines.append(SEP_EQ)
