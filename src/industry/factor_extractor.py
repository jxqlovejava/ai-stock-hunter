...
class FactorClassifier:
    ...
    KEYWORD_RULES: dict[str, tuple[str, str, str]] = {
        ...
        "扩产": ("基本面", "中长期", "不会自动消失"),
        "满产": ("基本面", "中长期", "不会自动消失"),
        # ── 供不应求信号 (文章共识 @LuBtc888 选股8标准 第④条) ──
        # 财报出现"供不应求/产线满产/下游排队/接受高溢价"字样 = 超级成长证据
        "供不应求": ("基本面", "中长期", "不会自动消失"),
        "满负荷": ("基本面", "中长期", "不会自动消失"),
        "满载": ("基本面", "中长期", "不会自动消失"),
        "产能满载": ("基本面", "中长期", "不会自动消失"),
        ...
        "排队提货": ("基本面", "中长期", "不会自动消失"),
        "提货排队": ("基本面", "中长期", "不会自动消失"),
        "接受高溢价": ("基本面", "中长期", "不会自动消失"),
        "高溢价采购": ("基本面", "中长期", "不会自动消失"),
        "溢价采购": ("基本面", "中长期", "不会自动消失"),
        "脱销": ("基本面", "中长期", "不会自动消失"),
        "一货难求": ("基本面", "中长期", "不会自动消失"),
        "供给": ("基本面", "中长期", "不会自动消失"),
    ...
    def infer_direction(cls, text: str) -> str:
        ...
        positive_kw = ["涨", "涨停", "上行", "上升", "走强", "突破", "改善",
                       ...
                       "需求", "支持", "加码", "战略", "驱动", "加速",
                       # 供不应求信号 (文章共识 选股8标准 第④条) → 超级成长偏多
                       "供不应求", "满产", "满负荷", "满载", "订单排满",
                       "订单饱满", "排产到", "排队提货", "提货排队", "脱销",
                       "一货难求", "高溢价", "溢价采购"]
